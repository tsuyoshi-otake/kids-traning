...
pickKokugo(p){const g=this.effectiveGrade(p),stage=this.topicStage(p,'kokugo');const L=[
...
];const pool=L.filter(x=>x.g<=g);const early=stage<=2?pool.filter(x=>x.g===1):(stage>=5?pool.filter(x=>x.g===g):pool);const use=early.length?early:pool;const it=use[this.rand(0,use.length-1)];const choose=stage>=5||(stage>=3&&Math.random()<0.5);const others=this.shuffle(use.filter(x=>x.k!==it.k));if(choose){const choices=this.shuffle([it.k].concat(others.slice(0,3).map(x=>x.k)));return{topic:'kokugo',mode:'choices',isKokugo:true,subtype:'kanji-choice',pre:it.pre,word:it.r,post:it.post,mean:'あう かんじを えらぼう',prompt:it.pre+it.r+it.post,answer:it.k,choices:choices,explanation:'「'+it.r+'」は 「'+it.k+'」と 書くよ。いみ：'+it.mean};}const choices=this.shuffle([it.r].concat(others.filter(x=>x.r!==it.r).slice(0,3).map(x=>x.r)));return{topic:'kokugo',mode:'choices',isKokugo:true,subtype:'reading',pre:it.pre,word:it.k,post:it.post,mean:it.mean,prompt:it.pre+it.k+it.post,answer:it.r,choices:choices,explanation:'「'+it.k+'」は 「'+it.r+'」と よむよ。いみ：'+it.mean};}
  ...
  ];const romaji=[['a','あ'],['i','い'],['u','う'],['e','え'],['o','お'],['ka','か'],['ki','き'],['ku','く'],['ke','け'],['ko','こ'],['sa','さ'],['shi','し'],['su','す'],['se','せ'],['so','そ'],['ta','た'],['chi','ち'],['tsu','つ'],['te','て'],['to','と'],['na','な'],['ni','に'],['ne','ね'],['no','の'],['ha','は'],['hi','ひ'],['fu','ふ'],['ho','ほ'],['ma','ま'],['mi','み'],['mu','む'],['me','め'],['mo','も'],['ya','や'],['yu','ゆ'],['yo','よ'],['ra','ら'],['ri','り'],['ru','る'],['re','れ'],['ro','ろ'],['wa','わ'],['n','ん']];let kinds=stage<=1?['hiragana']:stage===2?['hiragana','katakana','kotoba']:stage>=5?['kotoba','kotoba','alphabet']:['hiragana','katakana','alphabet','alphabet','kotoba'];const gm=this.effectiveGrade(p);if(gm>=3&&stage>=3)kinds=kinds.concat(stage>=5?['romaji','romaji','romaji']:['romaji','romaji']);const subtype=kinds[this.rand(0,kinds.length-1)];if(subtype==='kotoba'){const W=[
  ...
  ];const pool=D.filter(x=>x.g<=g);const early=stage<=2?pool.filter(x=>x.g===1):(stage>=5?pool.filter(x=>x.g===g):pool);const use=early.length?early:pool;const it=use[this.rand(0,use.length-1)];return{topic:'dokkai',mode:'choices',prompt:it.t+'　◆　'+it.q,answer:it.a,choices:this.pick4(it.a,it.c),explanation:it.e};}
  pickEigo(p){const stage=this.topicStage(p,'eigo');const mc=(pr,ans,pool,ex,speak)=>({topic:'eigo',mode:'choices',prompt:pr,answer:ans,choices:this.pick4(ans,pool),explanation:ex,speakChoices:!!speak});const Q=[];
    ...
    if(stage>=4){const S=[['I like apples.','わたしは りんごが すきです。'],['I have a dog.','わたしは いぬを かっています。'],['This is my book.','これは わたしの 本です。'],['I can swim.','わたしは およげます。']];Q.push(()=>wq(S));}
    if(stage>=5){const R=[['How are you?','I am fine.',['Good night.','My name is Ken.','Thank you.']],['What is your name?','My name is Ken.',['I am ten.','Good morning.','I like blue.']],['What color do you like?','I like blue.',['I am fine.','See you.','It is a dog.']]];Q.push(()=>{const it=R[this.rand(0,R.length-1)];return mc('しつもん「'+it[0]+'」に あう こたえは？',it[1],it[2],it[0]+' には '+it[1]+' と こたえるよ。',true);});}
