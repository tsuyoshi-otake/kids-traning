...
""";
    ...
    {
        return """
pickMul(p){const g=this.effectiveGrade(p),stage=this.topicStage(p,'mul'),m=p&&p.mastery?Number(p.mastery.mul):0.05;if(g>=3&&stage>=4&&m>=0.75&&Math.random()<0.25){const d=this.rand(2,5),q=this.rand(2,9),n=d*q,ans=''+q;return{topic:'mul',mode:'choices',op:'div',a:d,b:q,prompt:n+' ÷ '+d,answer:ans,choices:this.pick4(ans,[q+1,q-1,q+2,Math.max(1,q-2),d].map(String)),explanation:n+' ÷ '+d+' = '+q+'。'+d+' の まとまりが '+q+' こ。'};}let pairs;if(stage<=1||m<0.25){pairs=[[1,2],[2,1],[2,2],[1,3],[3,1]];}else if(stage<=2||m<0.45){pairs=[[1,2],[2,1],[2,2],[1,3],[3,1],[2,3],[3,2],[2,4],[4,2],[5,2],[2,5]];}else if(stage<=3||m<0.65){const tables=[1,2,3,4,5,10];pairs=[];tables.forEach(t=>{for(let x=1;x<=9;x++)pairs.push([t,x]);});}else{const tables=g>=3?[1,2,3,4,5,6,7,8,9]:[1,2,3,4,5,10];pairs=[];tables.forEach(t=>{for(let x=1;x<=9;x++)pairs.push([t,x]);});}const pair=pairs[this.rand(0,pairs.length-1)],a=pair[0],b=pair[1],ans=a*b;return{topic:'mul',mode:'choices',op:'mul',a:a,b:b,prompt:a+' x '+b,answer:''+ans,choices:this.pick4(''+ans,[ans+a,ans-a,ans+b,ans-b,a+b,Math.max(1,ans+a+b)].map(String)),explanation:a+' x '+b+' = '+ans+'。'+a+' こずつが '+b+' つ。'};}
""";
    }
