...
internal sealed class TrainingForm : Form
{
    ...
    private static string BuildProfileStorageScript()
    {
        ...
        (() => {
          ...
          const masteryKeys = ['add', 'sub', 'mul', 'clock', 'kokugo', 'hissan', 'moji', 'measure', 'kazu', 'shape', 'div', 'frac', 'chart', 'story'];
          const beginnerMastery = { add: .05, sub: .05, mul: .05, clock: .05, kokugo: .05, hissan: .05, moji: .05, measure: .05, kazu: .05, shape: .05, div: .05, frac: .05, chart: .05, story: .05 };
          const beginnerSettings = {
            count: 20,
            pass: 15,
            topics: { add: true, sub: true, mul: true, clock: true, kokugo: true, hissan: true, moji: true, measure: true, kazu: true, shape: true, div: true, frac: true, chart: true, story: true }
          };
