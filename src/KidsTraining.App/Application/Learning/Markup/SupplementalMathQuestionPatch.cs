...
internal static partial class LearningMarkupPatcher
{
  ...
  pickMoney(p){const g=this.effectiveGrade(p),stage=this.topicStage(p,'money'),coin=(values,prompt)=>{const total=values.reduce((a,b)=>a+b,0),near=[total+1,Math.max(1,total-1),total+5,total+10].filter(x=>x!==total).map(x=>x+'円');return{topic:'money',mode:'choices',isMoney:true,moneyPieces:values,prompt:prompt||'おかねは ぜんぶで いくら？',answer:total+'円',choices:this.pick4(total+'円',near),explanation:values.join('円 + ')+'円 = '+total+'円。'};},mc=(pr,ans,pool,ex)=>({topic:'money',mode:'choices',prompt:pr,answer:ans,choices:this.pick4(ans,pool),explanation:ex});let buckets;if(g<=1){buckets=[
    ...
    [()=>{const price=this.rand(2,9)*10,change=100-price;return mc('100円を もって '+price+'円の ものを かうと、おつりは？',change+'円',[price+'円',(change+10)+'円',Math.max(0,change-10)+'円'],'100−'+price+'='+change+'円。');}]
  ];}else{buckets=[
    ...
    [()=>{const paid=[500,1000][this.rand(0,1)],price=this.rand(2,Math.floor(paid/10)-1)*10,change=paid-price;return mc(paid+'円を はらって '+price+'円の ものを かうと、おつりは？',change+'円',[price+'円',(change+10)+'円',Math.max(0,change-10)+'円'],paid+'−'+price+'='+change+'円。');}]
  ...
  pickOrder(p){const stage=this.topicStage(p,'order'),cmp=(left,right,lv,rv)=>{const ans=lv===rv?'＝':(lv>rv?'＞':'＜');return{topic:'order',mode:'choices',prompt:left+' □ '+right+'　□に はいる しるしは？',answer:ans,choices:this.shuffle(['＞','＜','＝']),explanation:lv+' と '+rv+' を くらべると '+lv+ans+rv+'。'};};const buckets=[
    ...
    [()=>{const a=this.rand(1,9),b=this.rand(1,9),c=this.rand(1,9),ans=a+b+c;return{topic:'order',mode:'choices',prompt:a+' + ('+b+' + '+c+') は？',answer:''+ans,choices:this.pick4(''+ans,[a+b,a+c,b+c,ans+1].map(String)),explanation:'（ ）の なかを さきに。'+b+'+'+c+'='+(b+c)+'、'+a+'+'+(b+c)+'='+ans+'。'};}],
    [()=>{const b=this.rand(1,6),c=this.rand(1,6),a=this.rand(b+c+1,25),ans=a-(b+c);return{topic:'order',mode:'choices',prompt:a+' − ('+b+' + '+c+') は？',answer:''+ans,choices:this.pick4(''+ans,[a-b+c,a-b,ans+1].map(String)),explanation:'（ ）の なかを さきに。'+b+'+'+c+'='+(b+c)+'、'+a+'−'+(b+c)+'='+ans+'。'};}],
    [()=>{const a=this.rand(2,9),b=this.rand(1,9),c=this.rand(2,9),d=this.rand(1,9);return cmp(a+' + '+b,c+' + '+d,a+b,c+d);}],
    [()=>{const b=this.rand(2,7),c=this.rand(1,6),a=this.rand(b+c+2,30),inside=b+c,ans=a-inside;return{topic:'order',mode:'choices',prompt:a+' − ('+b+' + '+c+')　さいしょに けいさんするのは？',answer:b+' + '+c,choices:this.pick4(b+' + '+c,[a+' − '+b,a+' − '+c,a+' − '+inside]),explanation:'（ ）の なかの '+b+'+'+c+' を さいしょに けいさんする。こたえは '+ans+'。'};}]
  ];return this.pickStage(stage,buckets,0);}
""";
    }
}
