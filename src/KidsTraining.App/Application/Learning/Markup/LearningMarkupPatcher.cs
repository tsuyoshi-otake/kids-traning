...
internal static partial class LearningMarkupPatcher
{
    ...
    private static string PatchBeginnerProgression(string markup)
    {
        ...
        markup = ReplaceRequired(markup,
            ...
            "defaultSettings(){return {topics:{add:true,sub:true,hissan:true,mul:true,clock:true,kokugo:true,moji:true,measure:true,kazu:true,shape:true,div:true,frac:true,chart:true,story:true,bun:true,goi:true,dokkai:true,eigo:true,money:true,groups:true,order:true,soroban:true,seikatsu:true,shakai:true,rika:true,gijutsu:true,jouhou:true,sougou:true,keyboard:true,thinking:true},count:this.props.questionCount??30,pass:this.props.passLine??23,preferSchoolGrade:false};}",
            StringComparison.Ordinal);
        ...
        markup = PatchDataFigures(markup);
...
questionIdentity(q){const omit=new Set(['choices','explanation','display','sessionRole','difficulty']),normalize=v=>{if(Array.isArray(v))return v.map(normalize);if(v&&typeof v==='object'){const out={};for(const k of Object.keys(v).sort())if(!omit.has(k))out[k]=normalize(v[k]);return out;}return v;};return JSON.stringify(normalize(q));}
  ...
  rememberQuestion(p,q,key){const id=q.unitId||this.resolveUnitId(p,q.topic),stat=this.topicStat(p,id),fingerprint=this.questionFingerprint(key),recent=(Array.isArray(stat.recentQuestionFingerprints)?stat.recentQuestionFingerprints:[]).filter(item=>item!==fingerprint);recent.push(fingerprint);stat.recentQuestionFingerprints=recent.slice(-24);}
  ...
  sessionTopic(p,s,role){const target=this.refreshSessionTarget(p,s);if(role==='review'){const due=s.reviewTopics.filter(k=>this.topicDue(p,k));if(due.length){const topic=this.weightedPickExact(p,due),index=s.reviewTopics.indexOf(topic);if(index>=0)s.reviewTopics.splice(index,1);return topic;}}if(role==='target'||role==='exit'||role==='review')return target;const allowed=this.allowedTopics(p),practice=allowed.filter(k=>!this.topicComplete(p,k)||this.topicDue(p,k)),mixed=practice.filter(k=>k!==target),pool=mixed.length?mixed:(practice.length?practice:allowed),remaining=s.rolePlan.slice(s.questions.length).filter(r=>r==='mixed').length,current=s.mixedClusterTopic,canContinue=!!current&&(Number(s.mixedClusterLeft)||0)>0;if(canContinue){s.mixedClusterLeft=(Number(s.mixedClusterLeft)||0)-1;return this.weightedPick(p,[current]);}const used=Array.isArray(s.mixedClusterUsed)?s.mixedClusterUsed:[],fresh=pool.filter(k=>used.indexOf(k)<0),pickPool=fresh.length?fresh:(current?pool.filter(k=>k!==current):pool),topic=this.weightedPick(p,pickPool.length?pickPool:pool);let size=Math.min(5,Math.max(1,remaining));if(remaining-size===1&&size>3)size--;s.mixedClusterTopic=topic;s.mixedClusterLeft=size-1;s.mixedClusterUsed=fresh.length?used.concat([topic]):used;return topic;}
  ...
  buildSession(p,attempt){this.ensureLearningProfile(p);const n=this.total(),due=this.dueTopics(p),target=this.nextCurriculumTopic(p),reviewCount=due.length?Math.min(due.length,Math.max(1,Math.floor(n*.2))):0,targetTotal=Math.max(4,Math.floor(n*.25)),targetCount=targetTotal-1,mixedCount=n-reviewCount-targetCount-1,rolePlan=[];for(let i=0;i<reviewCount;i++)rolePlan.push('review');for(let i=0;i<mixedCount;i++)rolePlan.push('mixed');for(let i=0;i<targetCount;i++)rolePlan.push('target');rolePlan.push('exit');const session={questions:[],rolePlan:rolePlan,idx:0,correct:0,activeTargetTopic:target,targetTopics:[target],targetAsked:0,targetIndependent:0,reviewTopics:due.slice(),supportTopics:{},questionCounts:{},lastQuestionKey:'',mixedClusterTopic:'',mixedClusterLeft:0,mixedClusterUsed:[],attempt:attempt,startStars:p.stars,startXp:Number(p.xp)||0};session.questions.push(this.generateSessionQuestion(p,session,rolePlan[0]));return session;}
""";
    }
...
componentDidMount(){
    ...
    const settings={...def,...sourceSettings,topics:Object.fromEntries(Object.keys(def.topics).map(key=>[key,storedTopics[key]!==false])),count:count,pass:pass,schoolGrade:schoolGrade,preferSchoolGrade:preferSchoolGrade};
