...
questionIdentity(q){const omit=new Set(['choices','explanation','display','sessionRole','difficulty']),normalize=v=>{if(Array.isArray(v))return v.map(normalize);if(v&&typeof v==='object'){const out={};for(const k of Object.keys(v).sort())if(!omit.has(k))out[k]=normalize(v[k]);return out;}return v;};return JSON.stringify(normalize(q));}
  ...
  sessionTopic(p,s,role){const target=this.refreshSessionTarget(p,s);if(role==='review'){const due=s.reviewTopics.filter(k=>this.topicDue(p,k));if(due.length){const topic=this.weightedPickExact(p,due),index=s.reviewTopics.indexOf(topic);if(index>=0)s.reviewTopics.splice(index,1);return topic;}}if(role==='target'||role==='exit'||role==='review')return target;const allowed=this.allowedTopics(p),practice=allowed.filter(k=>!this.topicComplete(p,k)||this.topicDue(p,k)),mixed=practice.filter(k=>k!==target),pool=mixed.length?mixed:(practice.length?practice:allowed),previousQuestion=Array.isArray(s.questions)&&s.questions.length?s.questions[s.questions.length-1]:null,avoid=previousQuestion?(previousQuestion.unitId||this.resolveUnitId(p,previousQuestion.topic)):null;return this.weightedPick(p,pool,avoid);}
  ...
  buildSession(p,attempt){this.ensureLearningProfile(p);const n=this.total(),due=this.dueTopics(p),target=this.nextCurriculumTopic(p),reviewCount=due.length?Math.min(due.length,Math.max(1,Math.floor(n*.2))):0,targetTotal=Math.max(4,Math.floor(n*.25)),targetCount=targetTotal-1,mixedCount=n-reviewCount-targetCount-1,rolePlan=[];for(let i=0;i<reviewCount;i++)rolePlan.push('review');const body=new Array(targetCount+mixedCount).fill('mixed');for(let i=0;i<targetCount;i++){let pos=Math.floor((2*i+1)*body.length/(2*targetCount));while(pos<body.length&&body[pos]==='target')pos++;if(pos>=body.length){pos=body.length-1;while(pos>0&&body[pos]==='target')pos--;}body[pos]='target';}for(const role of body)rolePlan.push(role);rolePlan.push('exit');const session={questions:[],rolePlan:rolePlan,idx:0,correct:0,activeTargetTopic:target,targetTopics:[target],targetAsked:0,targetIndependent:0,reviewTopics:due.slice(),supportTopics:{},questionCounts:{},lastQuestionKey:'',attempt:attempt,startStars:p.stars,startXp:Number(p.xp)||0};session.questions.push(this.generateSessionQuestion(p,session,rolePlan[0]));return session;}
""";
    }
