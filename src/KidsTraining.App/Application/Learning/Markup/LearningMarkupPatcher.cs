...
questionIdentity(q){const omit=new Set(['choices','explanation','display','sessionRole','difficulty']),normalize=v=>{if(Array.isArray(v))return v.map(normalize);if(v&&typeof v==='object'){const out={};for(const k of Object.keys(v).sort())if(!omit.has(k))out[k]=normalize(v[k]);return out;}return v;};return JSON.stringify(normalize(q));}
  ...
  sessionStage(p,s,topic,role){const base=role==='review'?this.reviewStage(p,topic):this.topicStage(p,topic),id=this.resolveUnitId(p,topic),support=Math.max(0,Math.min(2,Number(s.supportTopics[id])||Number(this.topicStat(p,id).supportDepth)||0));return this.clamp(base-support,1,5);}
  ...
  generateSessionQuestion(p,s,role){const previous=this._progressionScope,scope={profile:p,values:new Map()};this._progressionScope=scope;try{const topic=this.sessionTopic(p,s,role),stage=this.sessionStage(p,s,topic,role),unit=this.curriculumUnit(this.resolveUnitId(p,topic)),deterministic=!!unit&&unit.generatorKey==='curriculum-bank'&&this.curriculumBankPool(unit,stage).length===1;let best=null,bestKey='',bestScore=Infinity;for(let attempt=0;attempt<24;attempt++){const candidate=this.genFor(topic,p,stage),key=this.questionIdentity(candidate),count=s.questionCounts[key]||0,score=count*2+(key===s.lastQuestionKey?1:0);if(count===0)return this.registerSessionQuestion(s,candidate,key,role);if(score<bestScore){best=candidate;bestKey=key;bestScore=score;}if(deterministic)break;}if(!best)throw new Error('Unable to generate a learning question');return this.registerSessionQuestion(s,best,bestKey,role);}finally{this._progressionScope=previous;}}
