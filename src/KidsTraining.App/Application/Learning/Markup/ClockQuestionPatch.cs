...
clockExplain(h,m,ask,a){if(ask==='hour')return 'みじかい はり が '+h+' を さして いるね。こたえは '+a+'。';if(ask==='minute')return 'ながい はり が さす すうじ ×5 が ふん。'+(m/5)+'×5='+m+'ふん。こたえは '+a+'。';return 'みじかい はり＝じ、ながい はり＝ふん。こたえは '+a+'。';}
  ...
  pickKazu(p){const g=this.effectiveGrade(p),stage=this.topicStage(p,'kazu');const mc=(pr,ans,pool,ex)=>({topic:'kazu',mode:'choices',prompt:pr,answer:''+ans,choices:this.pick4(''+ans,pool.map(String)),explanation:ex});const cmp=(a,b)=>({topic:'kazu',mode:'choices',prompt:'大きい ほうは？ '+a+' か '+b,answer:''+Math.max(a,b),choices:this.shuffle([''+a,''+b]),explanation:'くらべると '+Math.max(a,b)+' の ほうが 大きい。'}),count=(n,pr)=>({topic:'kazu',mode:'choices',isCount:true,count:n,prompt:pr||'まるは いくつ？',answer:''+n,choices:this.pick4(''+n,[n+1,Math.max(0,n-1),n+2].map(String)),explanation:'1つずつ かぞえると '+n+'こ。'});if(g<=1){const buckets=[
    ...
    [()=>{const total=this.rand(3,10),a=this.rand(1,total-1),b=total-a;return mc(total+'は '+a+'と いくつ？',b,[a,total,b+1],a+'と '+b+'で '+total+'。');},()=>{const a=this.rand(1,8),b=this.rand(1,10-a);return count(a+b,a+'こと '+b+'こを あわせると？');},()=>{const a=this.rand(1,8),b=this.rand(1,10-a),total=a+b;return{...mc(a+'と '+b+'で いくつ？',total,[Math.abs(a-b),total+1,Math.max(0,total-1)],a+'と '+b+'を あわせて '+total+'。'),subtype:'number-compose'};}],
    [()=>{const n=this.rand(11,20);return mc(n+' の 1つ まえは？',n-1,[n+1,n-2,n+10],n+' の 1つ まえは '+(n-1)+'。');},()=>{let a=this.rand(1,20),b=this.rand(1,20);while(a===b)b=this.rand(1,20);return cmp(a,b);},()=>{const start=this.rand(10,15),step=Math.random()<.5?1:2,answer=start+step*2;return{...mc(start+'、'+(start+step)+'、□、'+(start+step*3)+'　□は？',answer,[answer-step,answer+step,answer+1],step+'ずつ ふえる ならび。□は '+answer+'。'),subtype:'number-sequence'};}],
