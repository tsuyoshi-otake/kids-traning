...
// 1・3・4・6・8・10 で おわる 分は「ぷん」、それ以外は「ふん」と よむ。
  ...
  pickFrac(p){const stage=this.topicStage(p,'frac');const buckets=[
    ...
    [()=>{const d=this.rand(3,8),k=this.rand(1,d-1);return{topic:'frac',mode:'choices',isFracViz:true,fd:d,fn:k,prompt:'いろの ついた ところは ぜんたいの どれだけ？',answer:k+'/'+d,choices:this.pick4(k+'/'+d,[(d-k)+'/'+d,k+'/'+(d+1),d+'/'+d,(k+1)+'/'+d]),explanation:'ぜんたいを '+d+'つに 分けた '+k+'つ分で '+k+'/'+d+'。'};}],
