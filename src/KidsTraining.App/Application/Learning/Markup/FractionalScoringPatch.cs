...
internal static partial class LearningMarkupPatcher
{
    private static string PatchFractionalScoring(string markup)
    {
        ...
        markup = ReplaceBlock(markup, "finishNumeric(){", "\n  submit(ans){", "finishNumeric(value){const q=this.cur();this.finishScoredQuestion(q,this.state.numMiss||0,{userAnswer:value});}");
...
scoreForMistakes(misses){const n=Math.max(0,Math.min(3,Number(misses)||0));return n===0?1:(n===1?0.5:(n===2?0.25:0));}
  ...
  finishTyping(){const q=this.cur();this.finishScoredQuestion(q,this.state.typeMiss||0,{userAnswer:q.answer});}
""";
...
""";
  ...
  submitWrittenStep(val){if(this._answerBusy||this._terminalQuestionToken===this.currentQuestionToken())return;this._answerBusy=true;const q=this.cur(),plan=q&&typeof this.writtenArithmeticPlan==='function'?this.writtenArithmeticPlan(q):null,index=Number(this.state.waStep)||0,st=plan&&plan.steps[index],v=String(val!=null?val:this.state.input||'');if(!plan||!st){this._answerBusy=false;this.setState({waError:'この途中式を続けられません。答えと説明を確認します。'},()=>this.revealAnswer());return;}if(!this.writtenStepAnswerMatches(st,v)){const totalMiss=(this.state.waMistakes||0)+1;if(totalMiss>=3){this._answerBusy=false;this.exhaustQuestion(v);return;}this.sfx('wrong');const stepMiss=(this.state.waStepMiss||0)+1,upd={waHint:st.explain,input:'',combo:0,waMistakes:totalMiss,waStepMiss:stepMiss,waError:''};if(stepMiss>=2)upd.waStepChoices=st.phase==='decimal-result'?this.pick4(st.expect,[String(Number(st.expect)*10),String(Number(st.expect)/10),String(Number(st.expect)+1)]):this.numChoicesFor(st.expect);this.setState(upd,()=>{this._answerBusy=false;});return;}const next={input:'',waHint:'',waStepMiss:0,waStepChoices:null,waError:''},last=index>=plan.steps.length-1;if(last){this.setState(next,()=>{this._answerBusy=false;this.finishScoredQuestion(q,this.state.waMistakes||0,{viaSteps:true,perfectXp:14,comboXp:20,assistedXp:8});});return;}this.sfx('step');this.setState({...next,waStep:index+1},()=>{this._answerBusy=false;});}
""";

    private static string BuildFractionalRevealScript() => """
revealAnswer(){const q=this.cur();if(!q||!this.claimQuestionTerminal())return;const p=this.curP();this.recordEvidence(p,q,'revealed',0,{userAnswer:'わからない',mistakes:3});this.sfx('wrong');this.setState({screen:'feedback',combo:0,lastResult:{correct:false,revealed:true,q:q,userAns:'わからない',stars:0,combo:0,points:0,xp:0,levelUp:false},input:'',numChoices:null,hsStepChoices:null,waStepChoices:null,waHint:'',waError:'',typed:''});}
""";
}
