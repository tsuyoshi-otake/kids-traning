...
internal static partial class LearningMarkupPatcher
{
  ...
  historyQuestion(q){if(!q||typeof q!=='object')return null;const allowed=['id','unitId','topic','unitLabel','grade','unitGrade','difficulty','sessionRole','prompt','answer','choices','explanation','display','steps','activityPrompt','kind','mode','category'];const out={};for(const key of allowed)if(q[key]!=null)out[key]=q[key];return out;}
