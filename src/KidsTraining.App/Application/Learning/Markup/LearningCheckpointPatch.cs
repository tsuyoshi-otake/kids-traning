...
internal static partial class LearningMarkupPatcher
{
    private static string PatchLearningCheckpoint(string markup)
    {
        ...
        markup = ReplaceRequired(
            ...
            "componentWillUnmount(){if(this._keyActivate)document.removeEventListener('keydown',this._keyActivate);if(this._typeKeyHandler)document.removeEventListener('keydown',this._typeKeyHandler);this.flushProfilesSave(true);this.flushLearningCheckpoint(true);this.clearAutoAdvance();this.stopEnglishSpeech();if(window.__kidsTrainingPause)delete window.__kidsTrainingPause;if(window.__kidsTrainingDiscard)delete window.__kidsTrainingDiscard;if(window.__kidsTrainingReset)delete window.__kidsTrainingReset;}",
            StringComparison.Ordinal);
