...
internal static partial class LearningMarkupPatcher
{
    private static string PatchLearningCheckpoint(string markup)
    {
        ...
        markup = ReplaceRequired(
            ...
            "componentWillUnmount(){if(this._keyActivate)document.removeEventListener('keydown',this._keyActivate);if(this._typeKeyHandler)document.removeEventListener('keydown',this._typeKeyHandler);this.clearAutoAdvance();this.stopEnglishSpeech();if(window.__kidsTrainingPause)delete window.__kidsTrainingPause;if(window.__kidsTrainingDiscard)delete window.__kidsTrainingDiscard;if(window.__kidsTrainingReset)delete window.__kidsTrainingReset;}",
            StringComparison.Ordinal);
  ...
  clearLearningCheckpoint(){try{localStorage.removeItem(this.learningCheckpointKey());}catch(e){}}
  ...
  pauseLearning(notifyHost){this.clearAutoAdvance();const checkpoint=this.checkpointState();if(checkpoint&&!this.saveLearningCheckpoint(true)){if(this.state.screen==='feedback'&&this.state.lastResult&&this.state.lastResult.exhausted)this.scheduleAutoAdvance();return false;}if(notifyHost&&window.chrome&&window.chrome.webview&&typeof window.chrome.webview.postMessage==='function')window.chrome.webview.postMessage('kidsTraining.pause');return true;}
""";
    }
}
