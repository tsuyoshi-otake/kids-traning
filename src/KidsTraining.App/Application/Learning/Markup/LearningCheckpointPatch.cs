...
internal static partial class LearningMarkupPatcher
{
  ...
  validLearningCheckpoint(value){if(!value||value.version!==1||(value.screen!=='quiz'&&value.screen!=='feedback'))return false;const s=value.session,p=this.curP();if(!s||!p||value.profileName!==String(p.name||'')||!Array.isArray(s.rolePlan)||!Array.isArray(s.questions)||!Number.isInteger(s.idx)||s.idx<0||s.idx>=s.rolePlan.length||!s.questions[s.idx]||s.rolePlan.length<1||s.rolePlan.length>30)return false;const planned=[s.activeTargetTopic,s.mixedClusterTopic,...(Array.isArray(s.reviewTopics)?s.reviewTopics:[])];if(planned.some(k=>k&&!this.curriculumUnit(k)&&!this.topics[k]))return false;const unlocked=this.unlockedGradeTopics(p),unlockedIds=new Set(unlocked),topology=this.curriculumTopology(p),grades=[...topology.unitIdsByGrade.keys()].sort((a,b)=>a-b),minimumGrade=grades[0]||this.activeCurriculumGrade(p),activeGrade=this.activeCurriculumGrade(p),questionsValid=s.questions.every(q=>{if(!q||(q.topic&&!this.topics[q.topic]))return false;if(q.unitId)return unlockedIds.has(q.unitId);const grade=Number(q.grade);return Number.isInteger(grade)&&grade>=minimumGrade&&grade<=activeGrade;});if(!questionsValid)return false;const q=s.questions[s.idx],plan=typeof this.writtenArithmeticPlan==='function'?this.writtenArithmeticPlan(q):null,waStep=Number(value.waStep)||0;if(!plan)return waStep===0;const choices=value.waStepChoices;if(!Number.isInteger(waStep)||waStep<0||waStep>=plan.steps.length)return false;if(choices!=null&&(!Array.isArray(choices)||!choices.map(String).includes(String(plan.steps[waStep].expect))))return false;return true;}
