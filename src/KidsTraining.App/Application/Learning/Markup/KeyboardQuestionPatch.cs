...
internal static partial class LearningMarkupPatcher
{
    private static string PatchKeyboardQuestion(string markup)
    {
        ...
        markup = ReplaceRequired(
            ...
            "componentDidMount(){this._keyActivate=e=>{if((e.key==='Enter'||e.key===' ')&&e.target&&e.target.getAttribute&&e.target.getAttribute('role')==='button'){e.preventDefault();e.target.click();}};document.addEventListener('keydown',this._keyActivate);this._typeKeyHandler=e=>{if(e.repeat||e.isComposing||e.key==='Process'||e.ctrlKey||e.altKey||e.metaKey)return;if(!/^[a-zA-Z]$/.test(e.key))return;if(this.state.screen!=='quiz'||!this.state.session)return;const q=this.cur();if(!q||q.mode!=='type')return;e.preventDefault();this.typeKey(e.key);};document.addEventListener('keydown',this._typeKeyHandler);this._mathKeyHandler=e=>{if(e.repeat||e.isComposing||e.key==='Process'||e.ctrlKey||e.altKey||e.metaKey||this.state.screen!=='quiz'||!this.state.session)return;const roleButton=!!(e.target&&e.target.getAttribute&&e.target.getAttribute('role')==='button');if(roleButton&&(e.key==='Enter'||e.key===' '))return;const q=this.cur(),written=q&&typeof this.writtenArithmeticPlan==='function'?this.writtenArithmeticPlan(q):null,hissan=q&&q.mode==='hissan-steps',numeric=q&&q.mode==='num';if(!written&&!hissan&&!numeric)return;if(/^[0-9]$/.test(e.key)||(e.key==='.'&&written&&written.steps[Number(this.state.waStep)||0]?.phase==='decimal-result')){e.preventDefault();this.press(e.key);return;}if(e.key==='Backspace'||e.key==='Delete'){e.preventDefault();this.del();return;}if(e.key!=='Enter'||!this.state.input)return;e.preventDefault();if(written)this.submitWrittenStep();else if(hissan)this.submitHissanStep();else this.submitNum();};document.addEventListener('keydown',this._mathKeyHandler);\n    let profiles=this.state.profiles;",
            StringComparison.Ordinal);
