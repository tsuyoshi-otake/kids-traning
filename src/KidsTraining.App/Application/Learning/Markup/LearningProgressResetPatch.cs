...
internal static partial class LearningMarkupPatcher
{
  ...
  applyLearningReset(mode,options){if(mode!=='history'&&mode!=='full')return false;const opts=options||{},current=this.curP();if(!current)return false;this.ensureLearningProfile(current);const mastery={},skillStats={},unitStats={},blank={attempts:0,independent:0,assisted:0,revealed:0,errors:0,confidence:.05,reviewStep:0,retentionStep:0,lastAttemptAt:null,nextReviewAt:null,retentionStartedAt:null,masteredAt:null,level:1,stageAttempts:0,stageIndependent:0,evidenceWindow:[],supportDepth:0,consecutiveBlocks:0,recentQuestionFingerprints:[],multiplicationFacts:{}};for(const k of Object.keys(this.topics)){mastery[k]=.05;skillStats[k]={...blank,multiplicationFacts:{}};}for(const unit of this.curriculumCatalog())unitStats[unit.id]={...blank,multiplicationFacts:{}};const reset={...current,streak:0,stars:mode==='full'?0:(Number(current.stars)||0),xp:mode==='full'?0:(Number(current.xp)||0),mastery:mastery,skillStats:skillStats,unitStats:unitStats,cleared:{},learningSchema:7,progressResetAt:Date.now()},profiles=this.state.profiles.slice();profiles[this.state.profileIdx]=reset;const persisted=JSON.stringify(profiles);try{localStorage.setItem('kt_profiles_v1',persisted);this._lastSaved=persisted;}catch(e){this.setState({resetError:'学習データを保存できませんでした。もう一度お試しください。'});return false;}if(typeof this.clearLearningCheckpoint==='function')this.clearLearningCheckpoint();if(typeof this.clearLearningHistory==='function')this.clearLearningHistory();this.clearAutoAdvance();this._terminalQuestionToken='';this.sfx('select');this.setState({profiles:profiles,session:null,lastResult:null,input:'',combo:0,numMiss:0,numChoices:null,choiceMiss:0,hsStep:0,hsMistakes:0,hsStepMiss:0,hsStepChoices:null,waStep:0,waMistakes:0,waStepMiss:0,waStepChoices:null,waHint:'',waError:'',typed:'',typeMiss:0,resetConfirming:false,resetPin:'',resetError:'',screen:opts.screen||'parent'},()=>{if(opts.notifyHost&&window.chrome&&window.chrome.webview&&typeof window.chrome.webview.postMessage==='function')window.chrome.webview.postMessage('kidsTraining.resetApplied:'+mode);});return true;}
""";
    }
