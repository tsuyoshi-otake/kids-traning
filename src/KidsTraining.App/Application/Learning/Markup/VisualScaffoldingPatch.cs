...
internal static partial class LearningMarkupPatcher
{
    private static string PatchArithmeticVisuals(string markup)
    {
        ...
        markup = ReplaceRequired(markup,
            ...
            "\n      for(const row of measureRows){const cells=row.cells||[],values=cells.map(cell=>cell.text).filter(Boolean);if(q.isFracViz)row.ariaLabel='分数図。全体を '+(Number(q.fd)||2)+' つの同じ大きさに分け、そのうち '+(Number(q.fn)||1)+' つに色がついています。';else if(q.isOrder)row.ariaLabel=(q.od==='ひだり'?'左':'右')+'から数えて '+(Number(q.op)||1)+' 番目のマスを、太い枠とオレンジ色で示しています。';else if(q.isTape)row.ariaLabel='テープ図。左から '+values.join('、')+' の部分です。';else if(q.isTable)row.ariaLabel='表。'+row.label+' は '+values.join('、')+' です。';else if(q.isChart)row.ariaLabel='グラフ。'+row.label+' は '+cells.length+' ますです。';else if(q.isMoney)row.ariaLabel='お金の図。'+values.join('、')+'。';else if(q.isCount)row.ariaLabel='丸が '+cells.length+' こあります。';else row.ariaLabel=(row.label?row.label+' は ':'')+cells.length+' こを、枠のあるマスで示しています。';}\n      mulGroups.forEach((grp,index)=>{const count=(grp.cells||[]).length;grp.ariaLabel=count+' こずつのまとまり、'+(index+1)+' 組目。';});\n      for(const row of measureRows)for(const cell of (row.cells||[]))if(cell.text===undefined)cell.text='';\n      const plen=String(q.prompt||'').length;",
            StringComparison.Ordinal);
        ...
        markup = ReplaceBlock(
            ...
            "\n        if(S.numChoices)",
            "isAddViz=(q.topic==='add'||q.topic==='sub')&&this.topicStage(p,q.topic)<=2;if(isAddViz){const isSub=q.topic==='sub',base=isSub?Number(q.a||0):Number(q.n1||0),delta=isSub?Number(q.b||0):Number(q.n2||0),total=isSub?base:base+delta,frames=Math.max(1,Math.ceil(Math.max(base,total)/10));for(let f=0;f<frames;f++){const cells=[];let fill=0;for(let i=0;i<10;i++){const idx=f*10+i;let st='background:#fff;border:2px dashed #d8c4a0;';if(idx<base){fill++;if(isSub&&idx>=base-delta)st='background:linear-gradient(135deg,#ffdad4 0 42%,#d2503f 44% 56%,#ffdad4 58% 100%);border:2px solid #d2503f;';else st='background:#ff8a3d;border:2px solid #e07d2a;';}else if(!isSub&&idx<total){fill++;st='background:#2aa39a;border:2px solid #178a82;';}cells.push({style:'width:26px;height:26px;border-radius:50%;'+st});}addFrames.push({full:fill===10,cells:cells,ariaLabel:(isSub?'10こずつの枠。'+fill+'この丸のうち、斜線の丸はひいた数です。':'10こずつの枠。'+fill+'この丸で数を示しています。'),boxStyle:'display:inline-grid;grid-template-columns:repeat(5,26px);gap:6px;padding:10px;border-radius:14px;'+(fill===10?'border:3px solid #3aa655;background:#eafaef;':'border:3px solid #f0e2c8;background:#fff;')});}}\n        ");

        markup = ReplaceRequired(
            markup,
            "<div style=\"display:flex; flex-wrap:wrap; gap:16px; justify-content:center; align-items:center;\">\n                  <sc-for list=\"{{ addFrames }}\" as=\"fr\" hint-placeholder-count=\"2\">\n                    <div style=\"position:relative;\">\n                      <div style=\"{{ fr.boxStyle }}\">\n                        <sc-for list=\"{{ fr.cells }}\" as=\"cell\" hint-placeholder-count=\"10\">\n                          <div style=\"{{ cell.style }}\"></div>\n                        </sc-for>\n                      </div>\n                      <sc-if value=\"{{ fr.full }}\" hint-placeholder-val=\"{{ false }}\">\n                        <div style=\"position:absolute; top:-12px; right:-10px; background:#3aa655; color:#fff; border:2px solid #2f8a46; border-radius:14px; padding:2px 12px; font-size:18px; font-weight:900;\">10</div>\n                      </sc-if>\n                    </div>\n                  </sc-for>\n                </div>",
            "<div style=\"display:flex; flex-wrap:wrap; gap:16px; justify-content:center; align-items:center; max-width:100%; box-sizing:border-box;\">\n                  <sc-for list=\"{{ addFrames }}\" as=\"fr\" hint-placeholder-count=\"2\">\n                    <div role=\"img\" aria-label=\"{{ fr.ariaLabel }}\" style=\"position:relative;\">\n                      <div aria-hidden=\"true\" style=\"{{ fr.boxStyle }}\">\n                        <sc-for list=\"{{ fr.cells }}\" as=\"cell\" hint-placeholder-count=\"10\">\n                          <div aria-hidden=\"true\" style=\"{{ cell.style }}\"></div>\n                        </sc-for>\n                      </div>\n                      <sc-if value=\"{{ fr.full }}\" hint-placeholder-val=\"{{ false }}\">\n                        <div aria-hidden=\"true\" style=\"position:absolute; top:-12px; right:-10px; background:#3aa655; color:#fff; border:2px solid #2f8a46; border-radius:14px; padding:2px 12px; font-size:18px; font-weight:900;\">10</div>\n                      </sc-if>\n                    </div>\n                  </sc-for>\n                </div>",
            StringComparison.Ordinal);

        markup = ReplaceRequired(markup,
            "<div style=\"font-size:54px; font-weight:900; text-align:center; margin-bottom:6px; white-space:nowrap;\">{{ prompt }}</div>",
            "<div style=\"{{ promptStyle }}\">{{ prompt }}</div>\n            <sc-if value=\"{{ isShapeViz }}\" hint-placeholder-val=\"{{ false }}\">\n              <div role=\"img\" aria-label=\"{{ prompt }}\" style=\"display:flex; justify-content:center; margin:10px 0 12px; max-width:100%;\"><div aria-hidden=\"true\" style=\"{{ shapeStyle }}\"></div></div>\n            </sc-if>\n            <sc-if value=\"{{ isMulViz }}\" hint-placeholder-val=\"{{ false }}\">\n              <div style=\"display:flex; flex-wrap:wrap; gap:10px; justify-content:center; align-items:center; margin:8px auto 10px; max-width:720px; width:100%; box-sizing:border-box;\">\n                <sc-for list=\"{{ mulGroups }}\" as=\"grp\" hint-placeholder-count=\"6\">\n                  <div role=\"img\" aria-label=\"{{ grp.ariaLabel }}\" style=\"{{ grp.style }}\">\n                    <sc-for list=\"{{ grp.cells }}\" as=\"cell\" hint-placeholder-count=\"8\"><div aria-hidden=\"true\" style=\"{{ cell.style }}\"></div></sc-for>\n                  </div>\n                </sc-for>\n              </div>\n            </sc-if>\n            <sc-if value=\"{{ isMeasureViz }}\" hint-placeholder-val=\"{{ false }}\">\n              <div style=\"display:flex; flex-direction:column; gap:10px; align-items:flex-start; margin:8px auto 10px; width:100%; max-width:720px; min-width:0; box-sizing:border-box; overflow-x:auto; overscroll-behavior-inline:contain; padding:0 2px;\">\n                <sc-for list=\"{{ measureRows }}\" as=\"mrow\" hint-placeholder-count=\"2\">\n                  <div role=\"img\" aria-label=\"{{ mrow.ariaLabel }}\" style=\"display:flex; align-items:center; gap:10px; width:max-content; max-width:none;\">\n                    <div aria-hidden=\"true\" style=\"{{ mrow.labelStyle }}\">{{ mrow.label }}</div>\n                    <div aria-hidden=\"true\" style=\"{{ mrow.style }}\">\n                      <sc-for list=\"{{ mrow.cells }}\" as=\"mcell\" hint-placeholder-count=\"6\"><div aria-hidden=\"true\" style=\"{{ mcell.style }}\"></div></sc-for>\n                    </div>\n                  </div>\n                </sc-for>\n              </div>\n            </sc-if>",
            StringComparison.Ordinal);

        markup = ReplaceRequired(markup,
            "<div aria-hidden=\"true\" style=\"{{ mcell.style }}\"></div>",
            "<div aria-hidden=\"true\" style=\"{{ mcell.style }}\">{{ mcell.text }}</div>",
            StringComparison.Ordinal);

        markup = ReplaceRequired(markup,
            "<div style=\"position:relative; width:240px; height:240px; border:7px solid #3a3326; border-radius:50%; background:#fff; margin-bottom:8px;\">\n              <sc-for list=\"{{ clockMarks }}\" as=\"mk\" hint-placeholder-count=\"12\">\n                <div style=\"{{ mk.style }}\">",
            "<div role=\"img\" aria-label=\"時計。{{ clockAskLabel }}\" style=\"position:relative; width:min(240px,72vw); height:min(240px,72vw); aspect-ratio:1; box-sizing:border-box; border:7px solid #3a3326; border-radius:50%; background:#fff; margin-bottom:8px;\">\n              <sc-for list=\"{{ clockMarks }}\" as=\"mk\" hint-placeholder-count=\"12\">\n                <div aria-hidden=\"true\" style=\"{{ mk.style }}\">",
            StringComparison.Ordinal);

        markup = ReplaceRequired(markup,
            "<span style=\"{{ clockHourStyle }}\"></span>\n              <span style=\"{{ clockMinStyle }}\"></span>\n              <span style=\"position:absolute; left:50%; top:50%; width:18px; height:18px; background:#3a3326; border-radius:50%; transform:translate(-50%,-50%);\"></span>",
            "<span aria-hidden=\"true\" style=\"{{ clockHourStyle }}\"></span>\n              <span aria-hidden=\"true\" style=\"{{ clockMinStyle }}\"></span>\n              <span aria-hidden=\"true\" style=\"position:absolute; left:50%; top:50%; width:18px; height:18px; background:#3a3326; border-radius:50%; transform:translate(-50%,-50%);\"></span>",
            StringComparison.Ordinal);

        markup = ReplaceRequired(markup,
            "clockAskLabel=q.ask==='hour'?'みじかい はり ＝ なんじ':q.ask==='minute'?'ながい はり ＝ なんぷん（すうじ ×5）':'みじかい＝じ ／ ながい＝ふん';",
            "clockAskLabel=(Number(q.h)||0)+'時'+(Number(q.m)||0)+'分を示す時計。'+(q.ask==='hour'?'短い針の時刻を答えます':q.ask==='minute'?'長い針の分を答えます':'短い針が時、長い針が分です');",
            StringComparison.Ordinal);
