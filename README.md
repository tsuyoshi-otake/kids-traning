...
Curriculum scope, stable unit IDs, assessment modes, and official references are listed in [the grade 1-6 curriculum alignment](docs/curriculum-alignment.md).
...
- New profiles normally begin at the first unit of each independent subject lane with beginner confidence (`.05`), regardless of the registered school grade. Parents can optionally prefer the registered grade so each subject starts at its first available unit at or above that grade; this is not an upper limit, does not erase lower-grade progress, and can be turned off to restore the lower-grade frontier. Schema-v5 migration preserves stars, XP, history, historical clear flags, and legacy topic evidence while adding stable per-unit `unitStats`; rerunning the migration is idempotent.
- Math, Japanese, life/social/science, English, home economics, moral education, information/integrated study, special activities, and keyboard practice are independent ordered learning lanes through grade 6. The registered school grade is normally a parent-owned display and reporting attribute; the optional preference can use it only as the starting frontier, never as a question-selection cap. Completing difficulty 5 starts retention and automatically unlocks the next unit, including units above the registered grade; children cannot skip units manually.
...
- On-screen attention guidance is enabled by default. Starting or retrying a learning session requests camera permission through a user action, shows a mirrored preview and a smoothed face-position/size estimate, and gently displays `集中してね` only after four consecutive low samples with a 30-second cooldown. Detection runs locally at a bounded sampling rate, requests no microphone, saves and transmits no images, stops outside quiz/feedback screens, and can be turned off or stopped. It is explicitly an on-screen presence estimate, not identity, emotion, disability, or true concentration recognition; unavailable or denied detection never blocks learning.
- The catalog covers the existing grade 1-3 material and adds grade 4-6 mathematics, Japanese, science, social studies, English, home economics, moral education, integrated study/information, and special activities. Music, physical education, and arts and crafts remain excluded. Activities that cannot be judged objectively on screen—experiments, observation, speaking, cooking, sewing, and collaboration—use activity cards and reflection; the app does not claim to replace school instruction or practical work.
...
- Both PIN-protected in-app settings and the LAN parent page can set school grade 1-6, optionally prefer that registered grade as the starting frontier, choose a fixed question count from 10 through 30, and set a passing score from 1 through that selected count. The values share the atomic parent-settings store and live synchronization. Invalid values are rejected, missing legacy values default to grade 1 with grade preference off, and grade changes preserve all progress and affect display from the next session rather than the active question.
...
```powershell
rtk proxy powershell -NoProfile -ExecutionPolicy Bypass -File scripts\build-msi.ps1 -Version 1.22.0
```
