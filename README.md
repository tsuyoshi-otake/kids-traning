...
- The app builds `assets\kids-training.runtime.html` from the source template and learning app definition, then maps the local assets to the constrained virtual HTTPS host `learning.kidstraining.local` in WebView2. This secure context is required for camera access while keeping all learning content local.
...
- New or unstarted profiles start at grade 1, level 1, and beginner confidence (`.05`). Profiles saved by earlier releases are migrated in place: stars, streaks, mastery values, and historical clear flags are retained while five question-difficulty levels, a sixth retention stage, stage evidence counters, and review dates are added.
- Math, Japanese, life/social/science, moral education, information/integrated study, special activities, and supplementary English are independent ordered learning lanes. There is no cross-subject unlock chain, but each lane introduces only its next unfinished topic; integrated study follows the information-literacy foundation. The default grade 1 math order starts with numbers and shapes before addition/subtraction and later money/equal-group foundations; grade 2 starts with charts, clocks, arithmetic and measurement before written arithmetic, calculation order and multiplication; grade 3 starts with multiplication, division, shapes and advanced written arithmetic. Grade 1-2 use life studies, while grade 3 transitions to social studies and science and adds foreign-language activities. The setup UI intentionally offers only grades 1-3.
...
- On-screen attention guidance is enabled by default. Starting or retrying a learning session requests camera permission through a user action, shows a mirrored preview and a smoothed face-position/size estimate, and gently displays `画面を見てみよう` only after four consecutive low samples with a 30-second cooldown. Detection runs locally at a bounded sampling rate, requests no microphone, saves and transmits no images, stops outside quiz/feedback screens, and can be turned off or stopped. It is explicitly an on-screen presence estimate, not identity, emotion, disability, or true concentration recognition; unavailable or denied detection never blocks learning.
- The implemented grade 1-3 practice scope includes mental/written arithmetic, missing-number addition/subtraction, number sequences/composition, money, equal-group foundations, parentheses and inequality signs, multiplication tables, equal-sharing/equal-grouping division and remainders, place value through `1億`, multiplication through three-digit-by-one-digit and two-digit-by-two-digit forms, grade-3 `×10/×100/×1000/÷10`, soroban, flat/solid shapes and compass basics, grade-gated fractions/decimals, data classification/tables/scaled charts, tape-diagram word problems and □-equations, clock/time/weekday order, measurement units, kanji reading/selection, counters, greetings, feeling/reason inference, and kana/romaji. It also includes activity-based life studies, grade-3 social studies and science, moral education, information literacy, integrated inquiry, and special activities. Music, physical education, and arts and crafts are intentionally excluded. This is practice software, not a claim of complete textbook, handwriting, experiment, speaking-assessment, or school-curriculum equivalence.
...
- Japanese language coverage beyond kanji/kana lives in three dedicated topics: ぶん (bun: particles は・を・へ, punctuation and quotation marks, katakana loanword/onomatopoeia spelling, subject/predicate at grade 2+, modifiers at grade 3+), ことば (goi: antonyms, word groups, counters, greetings, and feeling/reason inference at all grades, plus proverbs/idioms and dictionary gojuon ordering at grade 3+), and よみとり (dokkai: 32 short reading-comprehension passages staged by grade). The canonical kanji bank exactly covers the MEXT grade allocation of 80 grade-1, 160 grade-2, and 200 grade-3 characters, with every character independently selectable as a reading or writing-choice target. The existing contextual entries remain as additional word and sentence variants.
- The grade-3 English topic is explicitly supplementary practice inspired by foreign-language activities: colors and numbers 1-10, everyday words, greetings, questions, answers, and basic self-expression. Pronunciation buttons use the browser's English (`en-US`) voice, and activity cards ask the learner to repeat, present, and conduct short role-play conversations; the app does not assess pronunciation or replace interactive speaking with a teacher or partner.
...
- The parent control page can also set a fixed question count from 10 through 30 and a passing score from 1 through that selected count. These values share the atomic parent-settings store, are validated by the application service, and are synchronized into WebView storage for the next learning session.
- Both the in-app and LAN parent dashboards offer PIN-confirmed reset choices. `学習履歴のみリセット` clears streak, mastery, five difficulty-level counters, sixth-stage retention evidence, achievements, review dates, and any active checkpoint while preserving level/XP/stars; `すべてリセット` additionally clears XP and stars. Both preserve the learner name, selected grade, profile color, parent PIN, question count, pass line, enabled topics, camera-guidance setting, and mute setting. A LAN reset is applied immediately when the learning WebView is active or stored as a pending reset for the next launch.
...
```powershell
rtk proxy powershell -NoProfile -ExecutionPolicy Bypass -File scripts\build-msi.ps1 -Version 1.21.0
```
...
- Local-only on-screen attention guidance: https://github.com/tsuyoshi-otake/kids-traning/issues/47
