...
- Topic progression is a clear-gated chain: every category unlocks only after its prerequisite category is cleared (mastery level 5, mastery >= 0.80). Existing sticky clear flags remain valid, so an upgrade never relocks previously cleared work. The chain is add → sub/moji → kazu/clock/story (after sub), kokugo and bun (after moji), goi (after bun), dokkai (after kokugo) → measure/chart (after kazu) → shape (after measure), and for grade 2+ hissan (after kazu) → mul (after hissan) → frac (after mul), with div at grade 3+ (after mul). Every category must be cleared to open the full set.
- Every topic uses the same five mastery levels: level 1 `<0.20`, level 2 `<0.40`, level 3 `<0.60`, level 4 `<0.80`, and level 5 `>=0.80`. The category list displays `Lv.n/5`. Stage-based generators choose the current level about 75% of the time and earlier material about 25% of the time, so new content is introduced gradually without dropping review questions. Existing continuous mastery values are preserved.
...
- Each learning session asks at least 20 questions and requires 15 correct answers to pass. Legacy saved settings of 10 questions are migrated automatically to 20 without resetting mastery or clear history.
